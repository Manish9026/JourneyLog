...
import './style.scss'
const Popup = forwardRef(({children,active,title,bodyClass,boxClass=""},ref) => {
...
{window.innerWidth>=700?
    ...
    <div ref={ref} onClick={(e)=>e.stopPropagation()} className={` ${boxClass} ${isActive?"bottom-[0]  opacity-100":"bottom-[-100vh] opacity-50"} transition-all duration-1000 ease box  p-1 absolute w-full   bg-blue-500    min-h-[200px]  `} >
        ...
        <span className='relative flex'>
            <h5>{title}</h5>
        <span className={`cursor-pointer  size-[25px] bg-slate-500 center absolute right-1 top-1 bg-slate-200/20 text-blue-200 rounded-full z-[30]`} onClick={()=>setIsActive(prev=>!prev)}> <FaXmark/></span>
        </span>
