...
const Popup = forwardRef(({children,flexibleBox, iconBox={},active,title,bodyClass,boxClass="",buttonToggle},ref) => {
  ...
  return (
    <div onClick={(e)=> {e.stopPropagation();setIsActive(0)}} className={`${bodyClass}  ${width<=700 && "bg-transparent"}  ${isActive?" visible opacity-100 z-[100]":" opacity-[50] invisible items-center"} flex justify-center tansition-all duration-700 left-0 fixed w-full top-0  bg-slate-300/20 min-h-full `}>
{window.innerWidth>=700?
    ...
    <div  onClick={(e)=>e.stopPropagation()} className={` ${boxClass} ${isActive?"bottom-[0]  opacity-100":"bottom-[-100vh] opacity-50"} transition-all duration-1000 ease box  p-1 absolute w-full   bg-blue-500    min-h-[200px]  `} >
        ...
        <span className='flex  left-0 top-0   w-full h-full'>
            {children}
        </span>
